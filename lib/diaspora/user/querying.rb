  ...
  module UserModules
    module Querying
      ...
      def visible_posts_sql(opts={})
        ...
        posts_from_others = posts_from_others.select(select_clause).limit(opts[:limit]).order(opts[:order_with_table]).where(Post.arel_table[opts[:order_field]].lt(opts[:max_time]))
        posts_from_self = posts_from_self.select(select_clause).limit(opts[:limit]).order(opts[:order_with_table]).where(Post.arel_table[opts[:order_field]].lt(opts[:max_time]))
        "(#{posts_from_others.to_sql}) UNION ALL (#{posts_from_self.to_sql}) ORDER BY #{opts[:order]} LIMIT #{opts[:limit]}"
      end
